...
import my.boxman.compat.android.widget.Toast;
...
public class myGameViewMap extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener {
    ...
    private Rect m_rPre;
    private BufferedImage bitNext;
    private BufferedImage bitPre;
    ...
    private BufferedImage bitUnDo;
    private BufferedImage bitReDo;
    private BufferedImage bitPreBK;
    private BufferedImage bitNextBK;
    private BufferedImage bitPreSkin;
    private BufferedImage bitNextSkin;
    private BufferedImage bitColorBK;
    int curButton = 0;

    private BufferedImage bitInvalid;  // 无效关卡图片
    ...
    public myGameViewMap() {
        ...
        addComponentListener(new ComponentAdapter() {
            ...
            public void componentResized(ComponentEvent e) {
                if (getWidth() > 0 && getHeight() > 0 && myMaps.curMap != null) {
                    setArena();
                    repaint();
                }
            }
    ...
    private void initView() {
        ...
        if (m_nArenaTop < 24) m_nArenaTop = 27;
        ...
        bitPreBK = ResourceLoader.getDrawable("prebk");
        bitNextBK = ResourceLoader.getDrawable("nextbk");
        bitColorBK = ResourceLoader.getDrawable("bk_color");
        bitPreSkin = ResourceLoader.getDrawable("preskin");
        bitNextSkin = ResourceLoader.getDrawable("nextskin");
    }
    ...
    public void setBackgroundColor(int argb) {
        setBackground(new java.awt.Color(argb, true));
    }
    ...
    public void mousePressed(MouseEvent e) {
        touchListener.onTouch(this, new MotionEvent(MotionEvent.ACTION_DOWN, e.getX(), e.getY(), e.getXOnScreen(), e.getYOnScreen()));
    }

    @Override
    public void mouseReleased(MouseEvent e) {
        touchListener.onTouch(this, new MotionEvent(MotionEvent.ACTION_UP, e.getX(), e.getY(), e.getXOnScreen(), e.getYOnScreen()));
        touchListener.mGestureDetector.onMouseUp(e);
    }
    ...
    public void mouseDragged(MouseEvent e) {
        touchListener.onTouch(this, new MotionEvent(MotionEvent.ACTION_MOVE, e.getX(), e.getY(), e.getXOnScreen(), e.getYOnScreen()));
    }
    ...
    private class TouchListener {
        ...
        public void applyZoom(float factor, float x, float y) {
            mCurrentMatrix.getValues(values);
            float scale = checkMaxScale(factor, values);
            mCurrentMatrix.postScale(scale, scale, x, y);
            reSetMatrix();
            invalidate();
        }

        private MyGestureListener mGestureDetector = new MyGestureListener();
        ...
        class MyGestureListener {
            ...
            public boolean onTouchEvent(MotionEvent e) {
                if (e.getActionMasked() == MotionEvent.ACTION_DOWN) {
                    ...
                    if (lastDownEvent != null && (now - lastDownTime < 350)
                            && Math.abs(e.getX() - lastDownEvent.getX()) < 20
                            && Math.abs(e.getY() - lastDownEvent.getY()) < 20) {
                    ...
                    longPressTimer = new javax.swing.Timer(500, evt -> {
                        isLongPressed = true;
                        onLongPress(e);
                    });
                    ...
                    if (lastDownEvent != null && (Math.abs(e.getX() - lastDownEvent.getX()) > 10 || Math.abs(e.getY() - lastDownEvent.getY()) > 10)) {
                        if (longPressTimer != null && longPressTimer.isRunning()) {
                            longPressTimer.stop();
                        }
                    }
                    ...
                    if (!isLongPressed && lastDownEvent != null) {
                        onSingleTapUp(e);
                    }
                }
                return true;
            ...
            public void onMouseUp(MouseEvent e) {
                if (e.getButton() == MouseEvent.BUTTON3) { // 右键直接触发长按操作
                    MotionEvent me = new MotionEvent(MotionEvent.ACTION_DOWN, e.getX(), e.getY(), e.getXOnScreen(), e.getYOnScreen());
                    onLongPress(me);
                }
            }
            ...
            public void onLongPress(MotionEvent e) {
                ...
                if (m_nArenaTop <= 0 && m_rUnDo.contains((int) e.getX(), (int) e.getY())) {  //长按 undo
                    ...
                    if (myMaps.m_Sets[13] == 0) {     // 需要在非“互动双推”模式下
                        ...
                        MyToast.showToast(m_Game, "请关闭“互动双推”模式再试！", Toast.LENGTH_SHORT);
                    }
...
//			}
        ...
        public boolean onTouch(Object v, MotionEvent event) {  //onTouchEvent
    ...
    private int getWall(int r, int c) {
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < m_nRows; i++) {
            for (int j = 0; j < m_nCols; j++) {
                ...
                // 因为皮肤具备透明“元素”，显示时，按层次画出，能够有一些特别效果，比如人的慢动作效果
                //第一、二层显示————地板、逆推水印
                if (ch != '_' && ch != '#') {
                    leftTmp = rt.left;
                    topTmp = rt.top;
                    ...
                    if (m_Game.bt_BK.isChecked()) {
                        leftTmp = rt.left;
                        topTmp = rt.top;
                        canvas.drawBitmap(myMaps.NituiPic, leftTmp, topTmp, myPaint);  //逆推水印————第二层
                    }
                ...
                if (ch == '.' || ch == '*' || ch == '+') {
                    leftTmp = rt.left;
                    topTmp = rt.top;
                    canvas.drawBitmap(myMaps.GoalPic, leftTmp, topTmp, myPaint);
                }

                //第四层显示————箱子或人
                switch (ch) {
                    case '#':   //墙壁属于第一层显示
                        if (myMaps.isSkin_200 == 0) { // 超简皮肤
                            leftTmp = rt.left;
                            topTmp = rt.top;
                            if (myMaps.isHengping) {   // 横屏时，对超简皮肤的墙壁做旋转处理
                                canvas.scale(-1, 1, rt.left + m_PicWidth / 2, rt.top + m_PicWidth / 2);
                                canvas.rotate(-90, rt.left + m_PicWidth / 2, rt.top + m_PicWidth / 2);
                            }

                            canvas.drawBitmap(myMaps.WallPic, leftTmp, topTmp, myPaint);
                            ...
                            leftTmp = rt.left;
                            topTmp = rt.top;
                            switch (getWall(i, j)) {  // 计算使用哪个“墙”
                            case 1: //仅左
                                canvas.drawBitmap(myMaps.WallPic_l, leftTmp, topTmp, myPaint);
                                break;
                            case 2: //仅上
                                canvas.drawBitmap(myMaps.WallPic_u, leftTmp, topTmp, myPaint);
                                break;
                            case 3: //左、上
                                canvas.drawBitmap(myMaps.WallPic_lu, leftTmp, topTmp, myPaint);
                                break;
                            case 4: //仅右
                                canvas.drawBitmap(myMaps.WallPic_r, leftTmp, topTmp, myPaint);
                                break;
                            case 5: //左、右
                                canvas.drawBitmap(myMaps.WallPic_lr, leftTmp, topTmp, myPaint);
                                break;
                            case 6: //右、上
                                canvas.drawBitmap(myMaps.WallPic_ru, leftTmp, topTmp, myPaint);
                                break;
                            case 7: //左、上、右
                                canvas.drawBitmap(myMaps.WallPic_lur, leftTmp, topTmp, myPaint);
                                break;
                            case 8: //仅下
                                canvas.drawBitmap(myMaps.WallPic_d, leftTmp, topTmp, myPaint);
                                break;
                            case 9: //左、下
                                canvas.drawBitmap(myMaps.WallPic_ld, leftTmp, topTmp, myPaint);
                                break;
                            case 10: //上、下
                                canvas.drawBitmap(myMaps.WallPic_ud, leftTmp, topTmp, myPaint);
                                break;
                            case 11: //左、上、下
                                canvas.drawBitmap(myMaps.WallPic_uld, leftTmp, topTmp, myPaint);
                                break;
                            case 12: //右、下
                                canvas.drawBitmap(myMaps.WallPic_rd, leftTmp, topTmp, myPaint);
                                break;
                            case 13: //左、右、下
                                canvas.drawBitmap(myMaps.WallPic_ldr, leftTmp, topTmp, myPaint);
                                break;
                            case 14: //上、右、下
                                canvas.drawBitmap(myMaps.WallPic_urd, leftTmp, topTmp, myPaint);
                                break;
                            case 15: //四方向全有
                                canvas.drawBitmap(myMaps.WallPic_lurd, leftTmp, topTmp, myPaint);
                                break;
                             default: //四方向全无
                                canvas.drawBitmap(myMaps.WallPic, leftTmp, topTmp, myPaint);
                                break;
                            ...
                            if (m_lWallTop && !m_Game.bt_Sel.isChecked()) { //是否画墙顶
                                ...
                                leftTmp = rt3.left;
                                topTmp = rt3.top;
                                canvas.drawBitmap(myMaps.WallPic_top, leftTmp, topTmp, myPaint);
                            }
                        }
                    ...
                    case '@':
                        if (d_Moves >= m_PicWidth) {
                            if (myMaps.isSimpleSkin || myMaps.isSkin_200 == 0) {  // 超简或简版皮肤
                                if (myMaps.isHengping) {   // 横屏时
                                    canvas.drawBitmap(myMaps.ManPic_r, leftTmp, topTmp, myPaint);
                                }
                                else {   // 竖屏时
                                    canvas.drawBitmap(myMaps.ManPic_u, leftTmp, topTmp, myPaint);
                                }
                            } else {     // 标准皮肤
                                leftTmp = rt.left;
                                topTmp = rt.top;
                                switch (PlayDir[myMaps.m_nTrun][myMaps.m_Sets[5]]) {
                                    ...
                                    case 3:  //右
                                        ...
                                        break;
                                    case 4:  //下
                                        canvas.drawBitmap(myMaps.ManPic_d, leftTmp, topTmp, myPaint);
                                        break;
                                    default: //上
                                        canvas.drawBitmap(myMaps.ManPic_u, leftTmp, topTmp, myPaint);
                                        break;
                    ...
                    case '+':
                        if (d_Moves >= m_PicWidth) {
                            if (myMaps.isSimpleSkin || myMaps.isSkin_200 == 0) {  // 超简或简版皮肤
                                if (myMaps.isHengping) {   // 横屏时
                                    canvas.drawBitmap(myMaps.ManGoalPic_r, leftTmp, topTmp, myPaint);
                                }
                                else {   // 竖屏时
                                    canvas.drawBitmap(myMaps.ManGoalPic_u, leftTmp, topTmp, myPaint);
                                }
                            } else {     // 标准皮肤
                                leftTmp = rt.left;
                                topTmp = rt.top;
                                switch (PlayDir[myMaps.m_nTrun][myMaps.m_Sets[5]]) {
                                    ...
                                    case 3:  //右
                                        ...
                                        break;
                                    case 4:  //下
                                        canvas.drawBitmap(myMaps.ManGoalPic_d, leftTmp, topTmp, myPaint);
                                        break;
                                    default: //上
                                        canvas.drawBitmap(myMaps.ManGoalPic_u, leftTmp, topTmp, myPaint);
                                        break;
                ...
                if (m_Game.bt_BK.isChecked()) {  //逆推
                    ...
                    if (myMaps.m_Sets[8] == 1) {  // 显示可达位置提示
                        ...
                        if ((m_boxCanMove2 || m_bManTo2 || m_bBoxTo2) && m_Game.mPathfinder.mark6[i][j]) { //穿越点提示
                            canvas.drawBitmap(myMaps.ChuanyuePic, leftTmp, topTmp, myPaint);
                        }
                    }
                    if (m_Game.m_bNetLock) {
                        if (m_Game.mark41[i][j] == 1) {        // 网位
                            canvas.drawBitmap(myMaps.WangweiPic, leftTmp, topTmp, myPaint);
                        } else if (m_Game.mark41[i][j] > 1) {  // 网口位
                            canvas.drawBitmap(myMaps.WangkouPic, leftTmp, topTmp, myPaint);
                        }
                    }
                ...
                if (m_Game.bt_Sel.isChecked()) { //计数状态
                    ...
                    if (m_Game.bt_BK.isChecked()) {  //逆推
                        ...
                        if (selNode2.r2 < 0 && m_iR == i && m_iC == j) {
                            canvas.drawBitmap(myMaps.SelPic, leftTmp, topTmp, myPaint);
                        }
                    } else {
        ...
        if (!m_Game.m_bACT_ERROR && (d_Moves < m_PicWidth || m_Game.m_nStep > 0)) {  // 有移动
            if (m_Game.b_nRow >= 0) {  // 移动箱子
                getRect(m_Game.b_nRow, m_Game.b_nCol, rt);
                leftTmp = rt.left;
                topTmp = rt.top;

                if (d_Moves*2 >= m_PicWidth) {
                    if (box_Goal[1]) canvas.drawBitmap(myMaps.BoxGoalPic, leftTmp, topTmp, myPaint);
                    else canvas.drawBitmap(myMaps.BoxPic, leftTmp, topTmp, myPaint);
                } else {
                    if (box_Goal[0]) canvas.drawBitmap(myMaps.BoxGoalPic, leftTmp, topTmp, myPaint);
                    else canvas.drawBitmap(myMaps.BoxPic, leftTmp, topTmp, myPaint);
                }
            ...
            if (myMaps.isSimpleSkin || myMaps.isSkin_200 == 0) {  // 超简或简版皮肤
                if (myMaps.isHengping)
                    canvas.drawBitmap(myMaps.ManPic_r, leftTmp, topTmp, myPaint);  // 使用横屏皮肤
                else
