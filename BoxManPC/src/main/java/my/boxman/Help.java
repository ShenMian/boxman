...
public class Help extends JFrame {
    ...
    private void initUI() {
        ...
        tv_help.setFont(new Font(Font.MONOSPACED, Font.PLAIN, 14));
        ...
        if (m_Num == 1) {  // 推关卡界面调用
            ...
            s.append('\n')
                    ...
                    .append('\n')
                    .append("【长按--墙壁】，正推时，提示没有移动过的箱子和没有用到的地板。例外，在关卡初始状态（移动步数为 0），显示该关卡的答案信息；逆推时，若“定位双推”开启，则切换是否显示“正逆推双目标点”").append('\n')
            ...
            s.append("====== 特别提醒 ======").append('\n')
                    ...
                    .append("  2、“快手”使用 SQLite 数据库管理关卡和答案数据，简称“关卡库”，关卡库及“快手”的其他数据均放置在电脑用户的“~/.boxman/”下，“DataBase/BoxMan.db”即为关卡库，关卡库中的数据不可手动修改，否则可能导致程序异常！但可以删除或改名（文件夹或关卡库），删除或改名后，关卡库（“快手”自带的原始数据）会自动重新生成；").append('\n')
                    .append("  3、“快手”支持自设皮肤，玩家可将自定义皮肤（PNG 图片）文档复制到“皮肤/”下，然后在程序里设置更换皮肤即可，“defskin.png”为默认皮肤（尺寸为 200 x 400，此文件可以删除，删除后会自动生成）为“快手”自带的默认皮肤，玩家可以参照其格式自己绘制，注意部分元素使用了透明背景。另外，“快手”还支持简版皮肤（200 x 350，仅含两个方向的仓管员图元）和超简皮肤（200 x 150，仅含一个墙体及两个方向的仓管员图元）；").append('\n')
